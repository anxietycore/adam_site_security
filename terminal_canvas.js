...
function updatePromptLine(){
  ...
  const notes = {
    ...
    'NOTE_002': { title: 'КОЛЬЦО СНА', author: 'tech-оператор U-735', content: ['Каждую ночь один и тот же сон.','Я в капсуле, фигура снаружи.','Она стучит по стеклу, но не пальцами.','Сегодня утром нашел царапины на руке.','ПРЕДУПРЕЖДЕНИЕ: ДОСТУПЕН ФАЙЛ ДЛЯ РАСШИФРОВКИ // ID: 0XC44 | Используйте DECRYPT 0XC44'] },
...
const decryptFiles = {
  '0XA71': {
    ...
    content: [
      ...
      '',
      'ХРОНОЛОГИЯ СОБЫТИЙ:',
      '14:32 - Запуск капсулы с экипажем из трёх человек.',
  ...
  '0XC44': {
    ...
    content: [
      ...
      '— Фантом (Субъект-095) возможно установил контакт с объектом',
      '',
      'СИСТЕМНЫЙ СТАТУС:',
  ...
  '0XD22': {
    ...
    content: [
      ...
      '',
      'СТАТИСТИКА:',
      'Из 39 субъектов, прошедших 3+ миссии:',
      '— 25 полностью потеряли личность',
      '— 9 проявили аномальную устойчивость (Фантом - один из них)',
...
'0XE09': {
  ...
  content: [
    ...
    '',
    'ХРОНОЛОГИЯ СОБыТИЙ:',
    '2003 — Попытка восстания и сабтоажа субъектов в лаборатории Jeneva',
    ...
    '',
    'ВАЖНЫЕ ДАННыЕ:',
    'REALITY-07 — единственная реальность где A.D.A.M. не существует и никогда не существовала.',
    '',
    'СИСТЕМНОЕ ПРЕДУПРЕЖДЕНИЕ:',
  ...
  'CORE': {
    ...
    content: [
      ...
      '> ВНИМАНИЕ. ФАЙЛ СКОМПРОМЕТИРОВАН. ДАННЫЕ В ФАЙЛЕ - НЕВЕРНЫ.',
      'ПРИРОДА ЯДРА:',
...
async function showLoading(duration = 2000, text = "АНАЛИЗ СИГНАЛА") {
  ...
  if (isTyping || operationManager.isBlocked()) return;
    ...
    if (degradation.level >= PSYCHO_BLOCK_START_LEVEL && degradation.psychoBlockActive && Math.random() < 0.25 && !intentionPredicted) {
  ...
  break;
      case 'decrypt':
        if (args.length === 0) {
          ...
          await typeText('Пример: decrypt 0X1A1', 'output', 12);
          break;
        }
